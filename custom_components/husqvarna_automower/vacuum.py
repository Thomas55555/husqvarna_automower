...
async def async_setup_entry(hass, entry, async_add_devices):
    ...
    async_add_devices(
        HusqvarnaAutomowerEntity(coordinator, idx) for idx, ent in enumerate(coordinator.data['data'])
    )
...
class HusqvarnaAutomowerEntity(HusqvarnaEntity, StateVacuumEntity, CoordinatorEntity):
    ...
    def battery_level(self):
        ...
        return max(0, min(100, self.coordinator.data['data'][self.idx]['attributes']['battery']['batteryPercent']))

    @property
    def device_state_attributes(self):
        """Return the specific state attributes of this mower."""
        self.mower_attributes = self.coordinator.data['data'][self.idx]['attributes']
        if self.coordinator.data['data'][self.idx]['attributes']['mower']['errorCodeTimestamp'] == 0:
            self.attr_errorCodeTimestamp = "-"
        else:
            self.attr_errorCodeTimestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime((self.coordinator.data['data'][self.idx]['attributes']['mower']['errorCodeTimestamp'])/1000))

        if self.coordinator.data['data'][self.idx]['attributes']['planner']['nextStartTimestamp'] == 0:
            self.attr_nextStartTimestamp = "-"
        else:
            self.attr_nextStartTimestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime((self.coordinator.data['data'][self.idx]['attributes']['planner']['nextStartTimestamp'])/1000))
